...
export function summarizeSockets(output: string): SocketSummary {
...
export function parseNetworkDeviceMetrics(content: string, socketSummary: SocketSummary) {
  return content
    .trim()
    .split("\n")
    .slice(2)
...
export class NetworkCollector {
  ...
  public async collect(): Promise<NetworkMetric[]> {
    ...
    return parseNetworkDeviceMetrics(content, socketSummary);
  }

  private async readSocketSummary() {
    try {
      const { stdout } = await execFileAsync("ss", ["-tunapH"]);
      return summarizeSockets(stdout);
    } catch {
      return {
        tcpConnections: 0,
        udpConnections: 0,
