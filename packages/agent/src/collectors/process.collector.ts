...
export function parseProcessMetricRecord(options: {
  pid: string;
  stat: string;
  statm: string;
  ...
  const { pid, stat, statm, uptimeSeconds, totalMemory } = options;
  ...
  const cpuPercent = Math.min(((utime + stime) / CLOCK_TICKS / lifetimeSeconds) * 100, 100);
...
export class ProcessCollector {
  ...
  private async readProcessMetric(
    ...
    try {
      ...
      return parseProcessMetricRecord({
        pid,
        stat,
        statm,
        uptimeSeconds,
