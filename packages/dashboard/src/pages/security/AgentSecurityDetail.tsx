  ...
  if (!profile) {
    return (
      <div className="rounded-[32px] border border-white/10 bg-white/[0.04] p-8 text-slate-300">
        Security details are not available yet.
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <section className="rounded-[32px] border border-white/10 bg-white/[0.04] p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.28em] text-cyan-200/70">Agent security detail</p>
          ...
          <div className="flex items-center gap-3">
            <Badge variant={badgeVariant(profile.agent.status)}>{profile.agent.status}</Badge>
            <Badge variant={badgeVariant(profile.overallStatus)}>{profile.overallStatus}</Badge>
            <Link
